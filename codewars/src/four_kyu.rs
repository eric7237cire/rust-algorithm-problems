...
fn mix(s1: &str, s2: &str) -> String {
    ...
    v.iter()
        .map(|tup| format!("{}:{}", tup.2, tup.1.to_string().repeat(*tup.0)))
        .collect::<Vec<String>>()
        .join("/")
}
...
mod decompose {
    ...
    impl Helper {
        fn decompose_helper(&mut self, n2: usize, level: usize) -> Option<usize> {
    ...
    fn decompose_also_too_slow(n: i64) -> Option<Vec<i64>> {
        ...
        for subset in (0..(1 << squares.len())).rev() {
            ...
            if sum == n * n {
                let ans: Vec<i64> = (0..n)
                    .filter(|i| (1i64 << i) & subset > 0)
                    .map(|i| indexes[i as usize])
                    .collect();
                return Some(ans);
            }
        }
