    ...
    run_cases(
        ...
        |reader, buffer| {
            ...
            for case_no in 1..=t {
                ...
                writeln!(buffer, "Case #{}: {}", case_no, ans).unwrap();
            }
        },
    );
}
...
fn solve(fly_radius: f64, racket_radius: f64, t: f64, chord_radius: f64, gap_len: f64) -> f64
{
    ...
    */
    ...
    for col in 0..num_cols {
        ...
        let lowest_arc_intersection_square_num =
            (((y_arc_intersection - (y1 + next_position2_inc)) / square_length).ceil()) as u64;
        ...
        for square_num in lowest_arc_intersection_square_num..num_squares {
            ...
            debug!(
                "  X1 arc {} x2 arc {}",
                x1_arc_intersection, x2_arc_intersection
            );

            //take care of the curve
            if x1 > x1_arc_intersection {
                x1_arc_intersection = x1
            }
            if x2 < x2_arc_intersection {
                x2_arc_intersection = x2
            }

            slice_miss_area += (y2 - y1) * (x2_arc_intersection - x1);

            let area_under_curve = integral_circle(x2_arc_intersection, inner_radius)
                - integral_circle(x1_arc_intersection, inner_radius);
